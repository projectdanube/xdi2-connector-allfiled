package xdi2.connector.allfiled;
...
public class ConnectServlet extends HttpServlet implements HttpRequestHandler {
	...
	static {
		...
		sampleEndpoint = "/xdi/allfiled"; 
	}

	public ConnectServlet() {

		this.graph = null;
		this.allfiledApi = null;
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("Request Access Token!".equals(request.getParameter("submit"))) {
			...
			try {
				...
				response.sendRedirect(this.getAllfiledApi().startOAuth(request, null, userXri));
				return;
			} catch (Exception ex) {
		...
		request.getRequestDispatcher("/AllfiledConnector.jsp").forward(request, response);
	}
	...
	public void setAllfiledApi(AllfiledApi allfiledApi) {

		this.allfiledApi = allfiledApi;
	}
}
